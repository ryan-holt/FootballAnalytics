...
--
-- Host: 127.0.0.1    Database: restfulexample
-- ------------------------------------------------------
...
/*!40000 ALTER TABLE `admin`
    DISABLE KEYS */;
INSERT INTO `admin`
VALUES (''harsohailB'', ''admin'', 4),
       (''ryanHolt'', ''ryanPassword'', 3);
/*!40000 ALTER TABLE `admin`
    ENABLE KEYS */;
UNLOCK TABLES;
...
VALUES (''1'', ''john'', NULL),
       ...
       (''8'', ''cool'', NULL),
       (''9'', ''gary'', NULL),
       (''harsohailB'', ''pass'', ''2020-02-24 11:57:24''),
       (''hello2'', ''world'', ''2020-02-24 02:17:01'');
/*!40000 ALTER TABLE `client`
    ENABLE KEYS */;
UNLOCK TABLES;
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `addAdmin`(IN username VARCHAR(20),
                                                      IN password VARCHAR(20),
                                                      IN permission_level Integer)
BEGIN
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `addClient`(IN username VARCHAR(20),
                                                       IN password VARCHAR(20))
BEGIN
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `getAdminByUsername`(
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `getAdmins`()
BEGIN
    SELECT * FROM admin;
END ;;
DELIMITER ;
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `getClientByUsername`(
    IN username VARCHAR(20)
)
BEGIN
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `getClients`()
BEGIN
    SELECT * FROM client;
END ;;
DELIMITER ;
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `updateAdminUsername`(IN old_username VARCHAR(20),
...
DELIMITER ;;
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `updateClientUsername`(IN old_username VARCHAR(20),
                                                                  IN new_username VARCHAR(20),
                                                                  IN password VARCHAR(20))
BEGIN
    UPDATE client
...
-- Dump completed on 2020-02-24 12:55:05
